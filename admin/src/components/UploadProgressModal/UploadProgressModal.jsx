...
const UploadProgressModal = ({ upload, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 px-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-[28px] border border-[#8b5cf6]/25 bg-[#150e2b] p-7 shadow-2xl shadow-black/60 md:p-8">
        ...
        {isError ? (
          <>
            ...
            <button
              type="button"
              onClick={onClose}
              className="mt-5 w-full cursor-pointer rounded-2xl border border-[#8b5cf6]/25 bg-[#8b5cf6]/10 px-5 py-3 text-sm font-black text-violet-200 transition hover:bg-[#8b5cf6]/20"
            >
              Close
            </button>
          </>
          ...
          <>
            ...
            <div className="mt-6 rounded-2xl border border-[#8b5cf6]/15 bg-black/30 px-5 py-5 text-center">
              ...
              <div className="h-2.5 w-full overflow-hidden rounded-full bg-black/50">
                <div
                  className={`h-full rounded-full transition-all duration-500 ${
                    isDone
                      ? "bg-emerald-400"
                      : "bg-gradient-to-r from-[#c4b5fd] via-[#8b5cf6] to-[#4338ca]"
                  }`}
                  style={{ width: `${percent}%` }}
                />
              </div>
